...
PRODUCTS (商品表 - 主表)存储二手商品的详细信息，这是交易的核心。SQLCREATE TABLE PRODUCTS (
    ...
    status VARCHAR(20) DEFAULT 'AVAILABLE' CHECK (status IN ('AVAILABLE', 'SOLD', 'PENDING')), -- 商品状态
...
PRODUCT_BIDS (商品出价表 - 从表)此表用于实现商品竞价功能，并满足“主从表结构，带业务外键”的要求。PRODUCTS 表是主表，PRODUCT_BIDS 表是从表，通过 product_id 业务外键关联。SQLCREATE TABLE PRODUCT_BIDS (
    bid_id INT IDENTITY(1,1) PRIMARY KEY,       -- 出价ID，自增主键
    product_id INT NOT NULL,                  -- 关联的商品ID (外键)
    bidder_user_id INT NOT NULL,              -- 出价用户ID (外键关联USERS表)
    bid_amount DECIMAL(10,2) NOT NULL,          -- 出价金额
    bid_time TIMESTAMP DEFAULT SYSTIMESTAMP,    -- 出价时间
    status VARCHAR(20) DEFAULT 'PENDING' CHECK (status IN ('PENDING', 'ACCEPTED', 'REJECTED')), -- 出价状态
    CONSTRAINT FK_BID_PRODUCT FOREIGN KEY (product_id) REFERENCES PRODUCTS(product_id) ON DELETE CASCADE, -- 业务规则：若商品被删除，则相关出价记录也级联删除
    CONSTRAINT FK_BID_USER FOREIGN KEY (bidder_user_id) REFERENCES USERS(user_id)
);
PRODUCT_BIDS 表通过 product_id 字段与 PRODUCTS 表建立了明确的主从关系。ON DELETE CASCADE 约束确保了当一个商品被删除时，所有相关的出价记录也会被自动清除，这是一种常见的数据完整性维护方式。这些表结构构成了二手交易系统的基础。USERS 表管理用户，CATEGORIES 表对商品进行分类，PRODUCTS 表存储商品信息，而 PRODUCT_BIDS 表则实现了核心的竞价功能，并满足了项目对主从表结构的要求。7. 功能模块 (4-6个示例)以下是建议的系统功能模块，涵盖了核心需求并利用了指定的技术。
...
描述： 用户可以对感兴趣的商品进行出价。卖家可以查看其商品的当前所有出价。此功能直接体现了 PRODUCTS (主表) 和 PRODUCT_BIDS (从表) 之间的关联。
相关表： PRODUCTS (主表), PRODUCT_BIDS (从表), USERS。
关键API端点：

POST /api/products/{productId}/bids：对指定商品进行出价，需要用户认证。
GET /api/products/{productId}/bids：查看指定商品的所有出价记录 (通常仅卖家或管理员可见)。
GET /api/users/me/bids：查看当前用户发起的所有出价记录。
...
<mapper namespace="com.example.yourproject.mapper.ProductMapper"> <resultMap id="ProductWithBidsResultMap" type="com.example.yourproject.model.Product">
    ...
    <select id="findProductWithBids" resultMap="ProductWithBidsResultMap">
        ...
        ORDER BY 
            b.bid_time DESC NULLS LAST -- 示例排序，将最新的出价放在前面，没有出价的商品也能正确显示
    </select>
</mapper>

这个XML映射演示了如何通过 <resultMap> 和 <collection> 标签来加载一个商品（主）及其所有相关的出价记录（从）。这种方式对于处理一对多或多对多关系非常有效，能够一次性获取所需数据，避免了N+1查询问题。
...
public class ProductController {
    ...
    public ResponseEntity<Product> getProductById(@PathVariable int productId) {
        Product product = productService.getProductWithBids(productId); // 调用获取商品及出价的方法
        if (product!= null) {
